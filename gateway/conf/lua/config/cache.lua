...
-- 缓存键前缀
local CACHE_PREFIX = "ai_gateway:"
local CACHE_KEYS = {
    namespaces = CACHE_PREFIX .. "namespaces",
    policies = CACHE_PREFIX .. "policies",
    upstreams = CACHE_PREFIX .. "upstreams",
    locations = CACHE_PREFIX .. "locations",
    matchers = CACHE_PREFIX .. "matchers"          -- 匹配器缓存
}
...
function _M.get_namespaces()
    return redis.get_json(CACHE_KEYS.namespaces)
end
...
function _M.get_policies()
    return redis.get_json(CACHE_KEYS.policies)
end
...
function _M.get_upstreams()
    return redis.get_json(CACHE_KEYS.upstreams)
end
...
function _M.get_locations()
    return redis.get_json(CACHE_KEYS.locations)
end
...
function _M.get_matchers()
    return redis.get_json(CACHE_KEYS.matchers)
end
...
function _M.get_all_configs()
    ...
    configs.namespaces = _M.get_namespaces() or {}
