...
function _M.handle_request()
    ...
    if not namespace_id then
        ngx.log(ngx.WARN, "No matching namespace found for request: ", request_info.path)
    ...
    ngx.log(ngx.INFO, "Matched namespace: ", namespace_id, " for request: ", request_info.path)
    ...
    if not policy_ok then
        ngx.log(ngx.WARN, "Policy enforcement failed: ", policy_message)
        metrics.record_request(namespace_id, request_info, 403, ngx.now() - start_time)
    ...
    ngx.log(ngx.INFO, "Policy enforcement passed for namespace: ", namespace_id)
...
function _M.handle_request_end()
    ...
    if namespace_id and request_id and start_time then
        ...
        -- 记录指标
        metrics.record_request(namespace_id, nil, status, response_time)
