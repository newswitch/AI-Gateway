...
local function build_target_url(upstream, request_info)
    local protocol = "http"
    if upstream.type == "https" then
        protocol = "https"
    end
    
    local port = upstream.port or 80
    if upstream.type == "https" and not upstream.port then
        port = 443
    end
...
local function set_proxy_headers(upstream, request_info)
    -- 设置Host头
    ngx.req.set_header("Host", upstream.address .. ":" .. (upstream.port or 80))
