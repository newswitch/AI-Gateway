...
local function health_check(upstream)
    ...
    local url = "http://" .. upstream.address .. ":" .. upstream.port .. upstream.health_check_url
    local res, err = http.get(url, {
        ["User-Agent"] = "AI-Gateway-Health-Check"
    })
    
    if not res then
        ngx.log(ngx.WARN, "Health check failed for ", upstream.name, ": ", err)
        return false
    end
...
local function get_healthy_upstreams(upstreams)
    ...
    for _, upstream in ipairs(upstreams) do
        if upstream.status == "enabled" and health_check(upstream) then
...
local function select_by_path_matching(namespace_id, request_info)
    ...
    for _, location in ipairs(locations) do
        if location.status == "enabled" and location.path then
            -- 简单的路径匹配（支持通配符）
            ...
            if string.match(request_info.path, "^" .. path_pattern) then
                ...
                for _, upstream in ipairs(upstreams) do
                    if upstream.id == location.upstream_id then
                        return upstream
                    end
                end
            end
...
function _M.get_upstream_info(upstream_id)
    ...
    for _, upstream in ipairs(upstreams) do
        if upstream.id == upstream_id then
            return upstream
        end
    end
