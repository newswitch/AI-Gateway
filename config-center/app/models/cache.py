...
class ConfigCache:
    ...
    async def set_namespace(self, namespace_id: int, namespace_data: Dict, ttl: int = None):
        """设置命名空间缓存"""
        try:
            cache_key = self._get_cache_key("namespace", str(namespace_id))
            await self.redis_client.set(cache_key, json.dumps(namespace_data), ex=ttl or self.cache_ttl['namespace'])
    ...
    async def set_matchers(self, namespace_id: int, matchers: List[Dict], ttl: int = None):
        ...
        try:
            ...
            await self.redis_client.set(cache_key, json.dumps(matchers), ex=ttl or self.cache_ttl['matchers'])
    ...
    async def set_upstream_server(self, server_id: int, server_data: Dict, ttl: int = None):
        ...
        try:
            ...
            await self.redis_client.set(cache_key, json.dumps(server_data), ex=ttl or self.cache_ttl['upstream'])
        except Exception as e:
    ...
    async def batch_get_namespaces(self, namespace_ids: List[int]) -> Dict[int, Dict]:
        ...
        try:
            # 批量从Redis获取
            cache_keys = [self._get_cache_key("namespace", str(ns_id)) for ns_id in namespace_ids]
    ...
    async def sync_from_mysql(self) -> bool:
        ...
        try:
            ...
            logger.info(f"成功同步 {len(namespaces)} 个命名空间, {len(matchers)} 个匹配器, {len(rules)} 个规则到Redis")
            return True
    ...
    async def create_namespace_dual_write(self, namespace_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            redis_task = self.redis_client.set(temp_key, json.dumps(namespace_data), ex=60)
    ...
    async def create_rule_dual_write(self, rule_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            redis_task = self.redis_client.set(temp_key, json.dumps(rule_data), ex=60)
            tasks.append(redis_task)
